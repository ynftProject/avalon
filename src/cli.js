...
program.command('nft-create-auction <author> <link> <price> <end>')
    .alias('nft-newauction')
    .description('create a new NFT auction')
    .action(function(author, link, price, end) {
        verifyAndSendTx('nftCreateAuction', author, link, price, end)
    }).on('--help', function(){
        writeLine('')
        writeLine('Arguments:')
        writeLine('  <author>: the nft author')
        writeLine('  <link>: the nft permlink')
        writeLine('  <price>: minimumm bid price')
        writeLine('  <end>: auction end timestamp')
        writeLine('')
        writeLine('Examples:')
        writeLine('  $ nft-create-auction alice nft1 20000 1653047510736 -F key.json -M alice')
        writeLine('  $ nft-newauction chris somejpeg 99900 1653242481985 -F key.json -M cat')
    })

program.command('nft-bid-auction <author> <link> <price>')
    .description('submit a bid in an NFT auction')
    .action(function(author, link, price) {
        verifyAndSendTx('nftBidAuction', author, link, price)
    }).on('--help', function(){
        writeLine('')
        writeLine('Arguments:')
        writeLine('  <author>: the nft author')
        ...
        writeLine('')
        writeLine('Examples:')
        writeLine('  $ nft-bid-auction alice nft1 20000 -F key.json -M alice')
    })
...
program.command('nft-create-order <author> <link> <price> <exp>')
    ...
    .action(function(author, link, price, exp) {
        ...
        writeLine('  $ nft-neworder chris somejpeg 99900 1653242481985 -F key.json -M cat')
    })
...
program.command('vote <author> <link> <downvote>')
    .description('vote for a content')
    .action(function(author, link, dv) {
        verifyAndSendTx('vote', author, link, dv)
    }).on('--help', function(){
        writeLine('')
        writeLine('Arguments:')
        writeLine('  <author>: the author of the comment to vote on')
        writeLine('  <link>: the identifier of the comment to vote on')
        writeLine('  <downvote>: boolean value of whether vote is a downvote or not')
        writeLine('')
        writeLine('Examples:')
        writeLine('  $ vote alice root-comment false -F key.json -M bob')
    })
